...
from lasotuvi.AmDuong import (ngayThangNamCanChi, timHoaLinh, thienCan, canChiGio)

def test_timhoalinh():
    print("\nTesting timHoaLinh function...")
    test_cases = [
        {
            'ngay': 7,
            'thang': 3, 
    ...
    for test in test_cases:
        try:
            ...
            # Get can chi components
            canNgay, chiNgay, canThang, chiThang, canNam, chiNam = \
                ngayThangNamCanChi(test['ngay'], test['thang'], test['nam'], 
                                 test['duongLich'], test['timeZone'])
            canGio, gioSinh = canChiGio(canNgay, test['gio'])
            # Test timHoaLinh function
            print(chiNam, gioSinh, test['gioiTinh'], thienCan[canNam]["amDuong"])
            vitriHoaTinh, vitriLinhTinh = timHoaLinh(chiNam, gioSinh, test['gioiTinh'], thienCan[canNam]["amDuong"])
            
            print(f"Vị trí Hỏa Tinh: {vitriHoaTinh}")
            print(f"Vị trí Linh Tinh: {vitriLinhTinh}")
            print("✓ Test passed")
